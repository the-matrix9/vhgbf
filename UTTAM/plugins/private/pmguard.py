...
from UTTAM.database.pmpermitdb import get_approved_users, pm_guard
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await UTTAM.set_block_message(UTTAM.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await UTTAM.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    await UTTAM.allow_user(chat_id)
